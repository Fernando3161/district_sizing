...
def get_pareto_front(df):
    """
    Identifies the Pareto optimal solutions from a DataFrame.

    Args:
        df (pandas.DataFrame): Input DataFrame containing the solutions.

    Returns:
        pandas.DataFrame: DataFrame containing the Pareto optimal solutions.
    """
    ...
    for i, row in df.iterrows():
        for j, other_row in df.iterrows():
            if i != j:
                if (row[0] > other_row[0] and row[1] > other_row[1]) or \
                        (row[0] >= other_row[0] and row[1] > other_row[1]) or \
                        (row[0] > other_row[0] and row[1] >= other_row[1]):
...
def plot_all_solutions(results, output_folder):
    """
    Plots the evolution of solutions.

    Args:
        results (pandas.DataFrame): Input DataFrame containing the results.
        output_folder (str): Output folder to save the plot.
    """
...
def plot_kpi_pairgrid(results, output_folder):
    """
    Creates a pair grid plot for key performance indicators (KPIs).
    ...
    optimal = optimal[[
        'ex_el_fraction', 'ex_heat_fraction', 'total_co2', 'total_system_costs'
    ]]
...
def plot_tech_pairgrid(results, output_folder):
    """
    Creates a pair grid plot for technology variables.
    ...
    optimal = results[results["Gen"] > 0]
    optimal = optimal[optimal["total_co2"] < 1e9]
    optimal = optimal[['PV', 'Wind', 'P2H', 'Sto']]
...
def plot_pareto(results, output_folder):
    """
    Plots the Pareto optimal solutions.
    ...
    fig.suptitle("Sizing of the Optimal System Configuration",
                 fontweight="bold",
                 fontsize=14, y=0.92)
    ...
    fit = {
        "x": np.linspace(0.01, max(df_pareto['total_system_costs']), 200),
...
def evaluate_results(filename="results_year.csv", output_folder=RESULTS_YEAR_DIR):
    """
    Evaluates the results from an optimization of an energy system.

    Args:
        filename (str): Filename of the results CSV file.
        output_folder (str): Output folder to save the plots.
    """
